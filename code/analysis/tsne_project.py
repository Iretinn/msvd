...
def visualize(features, labels):
    ...
    X_tsne = TSNE(n_components=2, random_state=33, init='pca', n_iter=5000, perplexity=10,
...
def main():
    ...
    titles = ['a', 'b', 'c', 'd']
    file_names = ["FQ2L_codebert_mmd_feature_large.pkl", "FQ2L_codebert_dann_feature_large.pkl", "FQ2L_madv_feature_large.pkl"]
    methods = ['CodeBERT_mmd', 'COdeBERT-dann', 'MSVD']
    # file_names = ["LF2Q_Sysevr_feature_large.pkl",
    #               "LF2Q_devign_feature_large.pkl",
    #               "LF2Q_codebert_feature_large.pkl",
    #               "LF2Q_madv_feature_large.pkl"]
    # methods = ['SySeVR', 'Devign', 'CodeBERT', 'MSVD']
    ...
    for index, file_name in enumerate(file_names):
        ...
        if index == 0:
            axs[0, index].legend(markerscale=5.0, loc='upper center', fontsize=16, bbox_to_anchor=(2.15, -0.01), ncol=2, frameon=False)
        ...
        axs[1, index].scatter(X_tsne_d0[:, 0], X_tsne_d0[:, 1], c='#E38D26', marker='^', label='Linux(Target)', s=1, alpha=0.5)
        axs[1, index].scatter(X_tsne_d2[:, 0], X_tsne_d2[:, 1], c='#91BFFA', marker='s', label='FFmpeg(Source)', s=1, alpha=0.5)
        axs[1, index].scatter(X_tsne_d1[:, 0], X_tsne_d1[:, 1], c='#832F87', marker='D', label='QEMU(Source)', s=1, alpha=0.5)
        if index == 0:
            axs[1, index].legend(markerscale=5.0, loc='upper center', fontsize=16, bbox_to_anchor=(2.15, -0.01), ncol=3, frameon=False)
